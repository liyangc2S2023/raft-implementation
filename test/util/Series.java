...
public class Series implements Serializable
{
    ...
    /** Creates a test series, given a list of test classes.
     ...
     may be modified.
     @param reps Number of times to run each test sequentially
    ...
    public Series(Class<? extends Test>[] tests, int reps)
    {
        // Check that the tests array is not null.
        if(tests == null)
            throw new NullPointerException("tests array is null");

        // Expanded tests array by x of reps
        this.tests = new Class[tests.length * reps];
        ...
        for(int test_index = 0; test_index < tests.length; ++test_index)
        {
            ...
            Arrays.fill(this.tests, test_index * reps, (test_index+1) * reps, tests[test_index]);
        }
    }
